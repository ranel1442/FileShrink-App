...
export default function PdfCompressor() {
  ...
  // הסטייט החדש שמחזיק *רק* את שם הקובץ בלי הסיומת
  ...
  const handleCompress = async () => {
    ...
    try {
      ...
      // חילוץ שם הקובץ המקורי ללא הסיומת (.pdf)
...
return (
    <div style={{ backgroundColor: 'rgba(255,255,255,0.7)', padding: '30px', borderRadius: '15px', boxShadow: '0 8px 32px rgba(0,0,0,0.05)', border: '1px solid rgba(255,255,255,0.4)' }}>
      <h2 style={{ color: '#e74c3c', marginBottom: '20px' }}>כיווץ קובץ PDF 📄</h2>
      ...
      {!downloadUrl ? (
        <>
          <div 
            {...getRootProps()} 
            style={{
              border: `2px dashed ${isDragActive ? '#28a745' : '#e74c3c'}`,
              borderRadius: '15px', padding: '40px', cursor: 'pointer',
              backgroundColor: isDragActive ? 'rgba(40, 167, 69, 0.1)' : 'rgba(231, 76, 60, 0.05)',
          ...
          {file && (
            <div className="action-area">
              ...
              <div className="quality-select-wrapper">
                ...
                >
                  <option value="large">גדול (איכות גבוהה)</option>
                  <option value="medium">בינוני (מומלץ)</option>
                  <option value="small">קטן (כיווץ מקסימלי)</option>
                </select>
              </div>
        ...
        <div className="action-area" style={{ backgroundColor: 'rgba(40, 167, 69, 0.1)', border: '1px solid rgba(40, 167, 69, 0.3)' }}>
          <h3 style={{ color: '#155724', marginBottom: '15px' }}>ה-PDF מוכן! 🎉</h3>
